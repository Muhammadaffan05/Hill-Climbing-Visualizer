...
def first_choice_hill_climbing(initial_position, step_size, max_iter):
    ...
    for i in range(max_iter):
        ...
        num_tries = 0
        while new_objective_value <= objective_value and num_tries < 10:
            new_position = current_position + np.random.uniform(-step_size, step_size)
            new_objective_value = objective_function(new_position)
            num_tries += 1
...
best_position, objective_values = first_choice_hill_climbing(initial_position, step_size, max_iter)
...
t.penup()
t.goto(0, 200)
t.write('First-choice Hill Climbing', align='center', font=('Arial', 20, 'bold'))
t.goto(0, -200)
...
t.goto(0, -240)
t.write(f'Step size: {step_size:.2f}, Max iterations: {max_iter}', align='center', font=('Arial', 14, 'normal'))
